...
| Integration/fault | temporary HOME/fake adapters, aliases, lock timeout, every build/publish/switch/health/rollback branch, stale journal recovery, PHP/Artisan argv and non-destructive unlink; shortcut pass-through (`a`, `am`, `tinker`, `ci`) with appended args/flags, global-flag stripping, and child exit-code propagation; user-systemd unit generation/lifecycle and managed scheduler cron entries |
