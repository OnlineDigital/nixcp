...
func newPHPCommand(runtime Runtime) *cobra.Command {
	cmd := &cobra.Command{
		Use:                "php [php-arguments...]",
		Short:              "Install, select, and run managed PHP versions",
		Long:               "Manage the PHP runtimes generated by NixCP. Supported nixpkgs versions are 8.2, 8.3, 8.4, and 8.5. `ncp php` with no subcommand runs the active PHP binary; project .php-version files take precedence over the shell and global defaults.",
		Example:            "  ncp php install 8.4\n  ncp php ext install redis\n  ncp php use --global 8.4\n  ncp php use 8.3\n  ncp php -v\n  ncp php uninstall 8.3",
		Args:               cobra.ArbitraryArgs,
		DisableFlagParsing: true,
		RunE:               func(c *cobra.Command, a []string) error { return dispatchPHP(c, runtime, passthroughArgs(c, a)) },
	}
	cmd.AddCommand(&cobra.Command{
		Use:     "install <version>",
		Short:   "Install a supported PHP version",
		Long:    "Add a supported nixpkgs PHP version to NixCP. The version is available to PHP, Composer, Artisan, and site PHP-FPM pools after the transaction completes.",
		Example: "  ncp php install 8.4",
		Args:    cobra.ExactArgs(1),
		RunE:    func(c *cobra.Command, a []string) error { return mutatePHP(c, runtime, "install", a[0]) },
	})
	cmd.AddCommand(&cobra.Command{
		Use:     "uninstall <version>",
		Short:   "Remove an unused managed PHP version",
		Long:    "Remove a PHP version from NixCP. A version used by a linked site cannot be removed; unlink or migrate those sites first. Removing the global default clears that default.",
		Example: "  ncp php uninstall 8.3",
		Args:    cobra.ExactArgs(1),
		RunE:    func(c *cobra.Command, a []string) error { return mutatePHP(c, runtime, "uninstall", a[0]) },
	})
	ext := &cobra.Command{Use: "ext", Short: "Manage curated nixpkgs PHP extensions", Long: "Install extensions from NixCP's explicit nixpkgs catalog. `apcu`, `bcmath`, `curl`, `gd`, `imagick`, `intl`, `mbstring`, `mysqli`, `pdo_mysql`, `pdo_pgsql`, `pdo_sqlite`, `redis`, `soap`, `sockets`, `xdebug`, `xml`, and `zip` work on PHP 8.2 through 8.5; `opcache` is separately available on PHP 8.2 through 8.4. Extensions apply to every installed PHP version that supports them.", Example: "  ncp php ext install redis\n  ncp php ext install imagick"}
	ext.AddCommand(&cobra.Command{Use: "install <name>", Short: "Enable a curated nixpkgs PHP extension", Long: "Enable a catalogued PHP extension for compatible installed PHP versions. Install a PHP version first.", Example: "  ncp php ext install redis", Args: cobra.ExactArgs(1), RunE: func(c *cobra.Command, a []string) error { return mutatePHP(c, runtime, "extension", a[0]) }})
	cmd.AddCommand(ext)
	use := &cobra.Command{Use: "use <version>", Short: "Select the active PHP version", Long: "Write the selected version to the current project's .php-version marker. With --global, select the default used by new shells instead.", Example: "  ncp php use 8.3\n  ncp php use --global 8.4", Args: cobra.ExactArgs(1), RunE: func(c *cobra.Command, a []string) error {
...
func sessionCommand(runtime Runtime) *cobra.Command {
	session := &cobra.Command{Use: "session", Short: "Print default PHP activation for new shells", Hidden: true, Args: cobra.ExactArgs(0), RunE: func(c *cobra.Command, _ []string) error {
...
func dispatchPHP(cmd *cobra.Command, runtime Runtime, args []string) error {
	...
	if len(args) == 2 && args[0] == "uninstall" {
		return mutatePHP(cmd, runtime, "uninstall", args[1])
	}
...
func mutatePHP(cmd *cobra.Command, runtime Runtime, action, raw string) error {
	...
	case "uninstall":
		...
		for _, site := range snap.Sites {
			if site.PHP == value {
				return apperrors.New("php_version_in_use", "PHP "+value+" is used by linked site "+site.Domain, "Relink or unlink that site before removing PHP "+value, apperrors.ExitCodePrecond)
			}
		}
		snap.Config.PHP.Installed = removeString(snap.Config.PHP.Installed, value)
		if snap.Config.PHP.GlobalDefault == value {
			snap.Config.PHP.GlobalDefault = ""
		}
		changed = true
	case "extension":
