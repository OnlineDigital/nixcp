...
func TestRenderIsDeterministicAndHTTPOnly(t *testing.T) {
	...
	if !strings.Contains(text, `users.users."u".homeMode = "0710";`) ||
		!strings.Contains(text, "systemd.services.nixcp-nginx-home-acl") ||
		!strings.Contains(text, "setfacl -m u:nginx:--x,g::---,m::--x") {
