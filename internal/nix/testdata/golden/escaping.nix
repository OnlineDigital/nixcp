...
{ lib, pkgs, ... }: {
  ...
  services.phpfpm.pools."nixcp-escape-test" = { user = "nixcp"; group = "users"; phpPackage = pkgs.php83.withExtensions ({ enabled, all }: enabled ++ [ all.intl all.redis ]); phpOptions = "upload_max_filesize = 2G\npost_max_size = 2G"; settings = { "listen.owner" = "nginx"; "listen.group" = "nginx"; "listen.mode" = "0660"; pm = "dynamic"; "pm.max_children" = 4; "pm.start_servers" = 2; "pm.min_spare_servers" = 2; "pm.max_spare_servers" = 2; "pm.max_requests" = 200; }; };
  services.nginx.virtualHosts."escape.test" = {
    ...
    locations."~ \.php$".extraConfig = "include ${pkgs.nginx}/conf/fastcgi.conf; fastcgi_pass unix:/run/phpfpm/nixcp-escape-test.sock;";
  };
}
